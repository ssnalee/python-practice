...
#화면 타이틀 설정
pygame.display.set_caption("SSna game")  #게임 이름
...
#배경 이미지 불러오기
# background = pygame.image.load('C:\\Users\\onebitten\\OneDrive\\바탕 화면\\practice\\python_ex\\pygame_basic\\bg.png')
...
enemy_y_pos = screen_height / 2 - enemy_height #화면 세로 크기 가장 아래에 해당하는 곳에 위치(세로)
...
while ruuning :
    ...
    for event in pygame.event.get() : #어떤 이벤트가 발생하였는가 ?
        if event.type == pygame.QUIT: #창 끄는 x버튼 클릭 시
            ruuning = False #게임 진행 False
    ...
    # screen.fill((255,255,255)) #배경 색 채워넣기 rgb색상
    ...
    # 출력할 글자, True, 글자 색상
    # 4.1초 => 4초로 하기위해 int로 감싸준 후 string으로 다시 변환
